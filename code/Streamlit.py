...
from Transform import ec_data as run_etl_pipeline

"""
Streamlit application that displays aircraft listing data by U.S. state.

- Triggers ETL pipeline to scrape and clean aircraft listings.
- Displays an interactive map with aircraft counts by state.
- Allows user to select a state to view detailed listings.
"""


def run_app():
    st.title("Aircraft Listings by State")
    with st.spinner("Scraping and cleaning aircraft listings..."):
        run_etl_pipeline()
    ...
    data = pd.read_csv(f'Cleaned_ASOlistings_{today_date}.csv')

    #gets the states location
    ...
    #sets the points on the map and their style
    ...
    #sets the hover info for the points
